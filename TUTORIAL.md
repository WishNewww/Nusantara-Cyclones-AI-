...
(Your Colab secret can still be *named* `GEMINI_API_KEY` in the 🔑
panel — that's just the secret's label. What matters is the
**environment variable** it gets assigned to, which must be
`GOOGLE_API_KEY`, per the note below.)
...
> **Why `GOOGLE_API_KEY` and not `GEMINI_API_KEY`:** `rag.py`'s LLM
> client, `langchain_google_genai.ChatGoogleGenerativeAI`, resolves its
> key by checking `GOOGLE_API_KEY` or `GEMINI_API_KEY` explicitly in
> `build_llm()` (preferring `GOOGLE_API_KEY` if both are set) — it does
> not fall back to Application Default Credentials the way some other
> Google client libraries do. Setting the env var under either name now
> works; `GOOGLE_API_KEY` is used above since it's the name the
> underlying `google-generativeai` SDK itself defaults to. Skipping
> this and leaving neither set produces a 401
> `ACCESS_TOKEN_TYPE_UNSUPPORTED` error (the SDK trying to fall back to
> OAuth/ADC) — if you ever see that error, check this cell first.
>
> `rag.py`'s `load_dotenv()` is a no-op here (no `.env` file exists on
> a fresh Colab runtime), which is fine — `os.environ` is already set
> directly by this cell.
...
| `401 ... ACCESS_TOKEN_TYPE_UNSUPPORTED` specifically | No API key reached the LLM client at all, so the SDK tried OAuth/ADC instead — usually because step 4 set an env var under a name `build_llm()` doesn't check, or Cell 4 was skipped/failed silently on a fresh runtime | Confirm `os.environ.get("GOOGLE_API_KEY")` (or `GEMINI_API_KEY`) prints a truthy value *in the same runtime* right before calling `ask_question()`; re-run Cell 4 if not |
