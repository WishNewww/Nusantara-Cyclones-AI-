...
build_vector.py
================
Builds the FAISS vector store for Nusantara Watch AI.
...
def load_region_csv(filename: str, region: str) -> pd.DataFrame:
    ...
    if not filepath.exists():
        raise FileNotFoundError(
            ...
            "Make sure both CSV files are placed under NusantaraWatchAI/data/."
        )
...
def classify_intensity_category(max_intensity: float | None) -> str:
    """Classify peak intensity into a general WMO-style band.

    Deliberately a *general* classification (Tropical Depression /
    Tropical Storm / Typhoon-equivalent), not an agency-specific Cat 1-5
    scale. Saffir-Simpson, the Australian scale, and JTWC categories all
    differ in their exact wind thresholds and are basin/agency-specific;
    this dataset doesn't indicate which agency's official category
    system applies to each observation, so asserting a precise Cat-3-vs-
    Cat-4 style number would overclaim precision the data doesn't
    support. The 34kt/64kt thresholds used here are the widely-used WMO
    general thresholds (tropical storm force / typhoon-hurricane force)
    that hold across basins, which keeps this defensible as a computed
    fact rather than an invented one.

    Computed here (pandas/plain Python, zero LLM involvement) so the
    LLM only ever narrates a pre-computed, retrievable fact instead of
    reaching into its own training-data knowledge of classification
    thresholds at generation time.
    """
    if max_intensity is None:
        return "Unknown (insufficient intensity data)"
    if max_intensity < 34:
        return "Tropical Depression"
    if max_intensity < 64:
        return "Tropical Storm"
    return "Typhoon / Severe Tropical Cyclone (equivalent)"
...
def build_cyclone_profile(name: str, region: str, group: pd.DataFrame) -> CycloneProfile:
    ...
    return CycloneProfile(
        ...
        intensity_category=classify_intensity_category(
            _safe_stat(group["INTENSITY"], np.max)
        ),
    )
...
def render_embedding_summary(profile: CycloneProfile) -> str:
    """Render a short, natural-language summary used ONLY for embedding.

    Why this exists (this is the fix for a real retrieval-quality bug):
    `sentence-transformers/all-MiniLM-L6-v2` truncates input at 256
    tokens and mean-pools whatever tokens survive into a single vector.
    The full profile+observation-history report is 800-3600+ words for
    93.8% of cyclones in this dataset, so almost none of a busy storm's
    history ever reaches the embedding model - and worse, the ~256
    tokens that DO survive are dominated by a rigid `Key : value`
    template that is IDENTICAL across all 2,303 documents (labels like
    "Start Date", "Duration", "Maximum Intensity"...). Under mean
    pooling, that shared boilerplate swamps the few tokens that actually
    distinguish one cyclone from another (mainly its name), so nearly
    every document ends up clustered close together in embedding space
    and similarity search degrades toward near-random - e.g. a query for
    "Cyclone ANN 1" can fail to retrieve the ANN 1 document at all.

    The fix: embed a short, plain-English paragraph that (a) fits
    comfortably under the model's context window with no truncation and
    (b) front-loads the cyclone's name and region in natural sentence
    form, so the tokens that actually distinguish this document make up
    a much larger share of the pooled vector - instead of embedding the
    same long, template-heavy report that gets shown to the LLM.
    """
    sentences = [
        f"Cyclone {profile.name} was a tropical cyclone in the {profile.region} "
        f"region, observed from {profile.start_time:%Y-%m-%d} to {profile.end_time:%Y-%m-%d} "
        f"({profile.duration_days} days, {profile.total_observations} observations)."
    ]

    if profile.max_intensity is not None:
        sentences.append(
            f"It reached a maximum intensity of {profile.max_intensity:.0f} knots "
            f"(minimum {_fmt(profile.min_intensity, 'knots', 0)}, "
            f"average {_fmt(profile.avg_intensity, 'knots')}), "
            f"classified as {profile.intensity_category}, "
            f"with an overall intensity trend of {profile.intensity_trend.lower()}."
        )
    else:
        sentences.append("Intensity data for this cyclone is unavailable.")

    sentences.append(
        f"It was first observed near latitude {profile.first_lat:.1f}, "
        f"longitude {profile.first_lon:.1f}, and last observed near "
        f"latitude {profile.last_lat:.1f}, longitude {profile.last_lon:.1f}, "
        f"moving through latitudes {profile.lat_min:.1f} to {profile.lat_max:.1f} "
        f"and longitudes {profile.lon_min:.1f} to {profile.lon_max:.1f}."
    )
...
def render_profile_section(profile: CycloneProfile) -> str:
    ...
    lines = [
        ...
        f"Intensity Category: {profile.intensity_category}",
    ]
    return "\n".join(lines)
...
def build_document(name: str, region: str, group: pd.DataFrame) -> Document:
    ...
    Document.page_content holds the SHORT embedding summary (see
    render_embedding_summary for why) - it is what gets embedded and
    indexed. The full, detailed profile + observation history report is
    stored in metadata["full_text"] and is what rag.py actually injects
    into the LLM's context after a document is retrieved. Retrieval
    quality and answer completeness are therefore optimized separately.
    """
    ...
    embedding_summary = render_embedding_summary(profile)
    ...
    metadata = {
        ...
        "intensity_category": profile.intensity_category,
        "full_text": full_text,
    }

    return Document(page_content=embedding_summary, metadata=metadata)
