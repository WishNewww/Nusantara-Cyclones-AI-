...
6.  Embed every Document with Google's Gemini embedding model and persist
    the resulting index to disk with FAISS so rag.py can load it later.

Run directly to (re)build the index:

    python build_vector.py

Requires a GOOGLE_API_KEY (or GEMINI_API_KEY) environment variable.
"""
...
# Gemini embedding model. "RETRIEVAL_DOCUMENT" task type is used at index
# time; rag.py uses "RETRIEVAL_QUERY" at query time for asymmetric search,
# which measurably improves retrieval quality for this embedding family.
...
def get_embeddings() -> GoogleGenerativeAIEmbeddings:
    ...
    Reads the API key from GOOGLE_API_KEY or GEMINI_API_KEY (handled
    automatically by the underlying client) - raises a clear error early
    if neither is set, instead of failing deep inside a batch embed call.
    """
    if not (os.getenv("GOOGLE_API_KEY") or os.getenv("GEMINI_API_KEY")):
        raise EnvironmentError(
            "No Gemini API key found. Set GOOGLE_API_KEY (or GEMINI_API_KEY) "
            "in your environment or in a .env file before running build_vector.py."
        )
...
def build_and_persist_index(
    ...
    logger.info("Embedding %d document(s) with Gemini (%s)...", len(documents), EMBEDDING_MODEL)
